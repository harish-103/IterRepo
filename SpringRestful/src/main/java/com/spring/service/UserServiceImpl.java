...
public class UserServiceImpl implements UserService {
		...
		User newUser =UserUtils.convertUserEntityToUser(userRepository.findByUserName(user.getUserName()));
			...
			if (newUser.getUserName().equals(user.getUserName()))
			{
				if(newUser.getPassword().equals(user.getPassword()))
				{
					...
					userEntity =userRepository.save(userEntity);
					return "{\"result\": \"Successfull\",\"auth-token\":\""+sessionId+"\"}";
				}
				else 
				{
					return "{\"result\": \"THe Password is Incorrect,Please Try again\"}";
				}
			}	
		else 
			return "{\"result\": \"User is invalid\"}";
	...
	public String logout(String authToken) 
	{
		...
		{
			return "{\"result\": \"The Session-Id is invalid\"}";
		}
		else
		{
		...
		userEntity =userRepository.save(userEntity);
		return "{\"result\": \"Congratulations!!!,Sucessful\"}";
		}
	}
